...
-- can do. TNTICK read at the instant of the write is exact and agrees.
--
...
local function rawtick() return sp:readv_u8(TNTICK) end

-- The schedule is written in EXTENDED ticks. TNTICK is eight bits and wraps
...
_G._rg_drive = emu.add_machine_frame_notifier(function()
    ...
    -- the right tick. The host is chosen by TNENT's role bit.
    -- Only once the match has started: before it, TNENT is zero on BOTH
...
_G._rg = sp:install_write_tap(0x2C, 0x2C, "cxclr", function(off, data, mask)
    ...
    --
    -- THE PHASE COMES FROM TNENT, NOT FROM CLOCK. It used to be `CLOCK & 3`,
    -- and that is the same bug the ROM had: ClrGam clears $82-$A2 and CLOCK is
    -- $86, so every SELECT advance re-phases it. TNENT's bits 6-7 are stepped
    -- by TNPHI once per frame and there is no instruction anywhere in Combat
    -- that writes them. TNSHIM steps the phase on its way out, so the boundary
    ...
    local ph = sp:readv_u8(TNENT) & 0xC0
    ...
    if snap == nil and sp:readv_u8(TNTICK) >= SNAPTICK and ph == 0x40 then
        local b = {}
        ...
        snap = string.format(
            ...
            sp:readv_u8(TNTICK), sp:readv_u8(TNCRCV), sp:readv_u8(TNSWA),
            sp:readv_u8(TNSWB), table.concat(b, " "))
        snaploc = string.format("LOCAL ring=$%02X hw=$%02X",
            sp:readv_u8(TNRING), sp:readv_u8(0x0282))
    end
end)
...
-- console ignored it".
local swseen, hwseen = {}, {}
_G._rg_sw = sp:install_write_tap(TNSWB, TNSWB, "voswb", function(off, data, mask)
    swseen[data] = (swseen[data] or 0) + 1
    -- Only once the match is running. TNSWB is zero out of the RAM clear and a
    ...
    if firstsel == nil and (data & 0x02) == 0
       and (sp:readv_u8(TNENT) & 0x02) ~= 0 then
...
_G._rg_stop = emu.add_machine_stop_notifier(function()
    print(string.format(
        ...
        sp:readv_u8(TNRWAT), frames, stalls))
