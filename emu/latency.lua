...
--
-- probe.asm publishes a step id into TNST and the frames that step took into
-- TNFCNT. A step id is EVEN when a transaction is launched and ODD when it
...
local function report()
    ...
    print(string.format("FRAMES %d  ROUNDS %d  ERR $%02X",
                        frames, rounds, sp:readv_u8(TNERR)))
...
_G._lat_st = sp:install_write_tap(TNST, TNST, "step", function(off, data, mask)
    if data == 0xFF then
        fails = fails + 1
    ...
    -- FNRXLO/FNRXHI.
    if data % 2 == 1 and NAME[data] then note(NAME[data], sp:readv_u8(TNFCNT)) end
end)
