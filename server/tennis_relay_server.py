#!/usr/bin/env python3
"""tennis_relay_server.py -- the relay for networked Atari 2600 Tennis.

Port 9602 -- Combat's room is 9600, Dragster's 9601. A fork of the Intellivision family's
...
split frame in -- the 128 bytes of console RAM are Tennis's, and the reply
...
Usage: tennis_relay_server.py [--port 9602] [--delay 2] [--lobby-url ...]
"""
...
class Server:
    ...
    def on_hello(self, c, payload):
        ...
        #
        # COMBAT REFUSED SECAM FOR A REASON THAT IS NOT TENNIS'S. Its two
        # tanks are the same shape and are told apart only by colour, so on a
        # SECAM set two players could be looking at identical sprites. Tennis
        ...
        c.tv = payload[1]
        if c.tv == TV_SECAM:
            log("%s: refusing a SECAM console -- no SECAM Tennis image "
...
def main():
    ...
    ap.add_argument("--variation", type=int, default=1,
                    help="the variation a match starts on. ONLY THE ODD ONES "
                         "are offered: $80 runs 0-3 and $81 is $80 AND 1, "
                         "which is 0 for one player against the computer and "
                         "1 for two humans. A networked match with the "
                         "computer playing one side is not a match.")
    ap.add_argument("--lobby-url", default="",
                    help="e.g. https://lobby.fujinet.online/server; off by default")
    ap.add_argument("--lobby-appkey", type=int, default=26)
    ap.add_argument("--lobby-serverurl", default="TCP://fujinet.online:9602/")
    ap.add_argument("--lobby-client-url",
                    default="TNFS://apps.irata.online/Atari2600/Games/Tennis.bin")
    ap.add_argument("--game-name", default="Tennis")
    ap.add_argument("--server-name", default="Tennis Netplay")
    ...
    # in the ROM is the computer-opponent block at $F212, which $F210 skips
    # when $81 is nonzero.
    if args.variation not in (1, 3):
        sys.exit("tennis_relay: variation must be 1 or 3 -- the two-human "
                 "games. $81 is $80 AND 1, and it is 0 for the variations "
                 "where the computer plays one side.")
    Server(args).run()
